...
$message = "Admin Successfully Updated  Agent info ";
$response = [];
...
if (empty($name) || empty($phone) || empty($address)) {
    ...
    $check->bind_param("ss", $name, $agent_id);
    $check->execute();
    $check->store_result();
    ...
    try {
        ...
        // Prepare SQL
        $stmt = $conn->prepare(
            "UPDATE agents SET name = ?, email = ?, phone = ?, address = ? WHERE agent_id = ?"
        );

        $stmt->bind_param("sssss", $name, $email, $phone, $address, $agent_id);
...
$update->bind_param("sss", $name, $agent_id, $target_tb);
$update->execute();

            // notification Insert
            $stmtNotify = $conn->prepare("INSERT INTO notifications (actor_type, action,target_table,target_id, message) VALUES (?, ?, ?, ?, ?)");
            $stmtNotify->bind_param('sssis', $actor_type,  $action_type, $target_tb, $agent_id, $message);
            $stmtNotify->execute();
